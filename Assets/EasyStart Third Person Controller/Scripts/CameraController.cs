...
public class CameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //if( canZoom && Input.GetAxis("Mouse ScrollWheel") != 0 && (Camera.main.fieldOfView>=40 && Camera.main.fieldOfView<=70))
        //    Camera.main.fieldOfView -= Input.GetAxis("Mouse ScrollWheel") * sensitivity * 2;
        // You can use Mathf.Clamp to set limits on the field of view

        // did it because zoom in/out was not limited
        if (canZoom && Input.GetAxis("Mouse ScrollWheel") <0 && Camera.main.fieldOfView < 70)
            Camera.main.fieldOfView = Camera.main.fieldOfView +1 ;

        if (canZoom && Input.GetAxis("Mouse ScrollWheel") >0 && Camera.main.fieldOfView >40)
            Camera.main.fieldOfView = Camera.main.fieldOfView - 1 ;
